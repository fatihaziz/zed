...
usage() {
    echo "Usage: $0 [-w|--workspace <workspace>] [-b|--bin <bin>] [-k|--skip-check] [-n|--skip-clean] [-r|--root] [--export-only]"
}
...
OPTIONS=$(getopt -o w:b:knre --long workspace:,bin:,skip-check,skip-clean,root,export-only -- "$@")
...

{
  if ! $EXPORT_ONLY_FLAG; then
    echo "Releasing with options: workspace: $WORKSPACE, bin: $BIN, skip_check: $SKIP_CHECK_FLAG, skip_clean: $SKIP_CLEAN_FLAG, selective_build: $SELECTIVE_BUILD_FLAG, root_build: $ROOT_BUILD_FLAG"
    ...
    if $ROOT_BUILD_FLAG ; then
      cargo build -r --offline --target $TARGET || {
      ...
      if $BIN_BUILD_FLAG ; then
        cd $WORKSPACE
        cargo build --bin $BIN -r --offline --target $TARGET || {
          echo -e "\033[31mCargo build failed for binary $BIN! ❌\033[0m"
          exit 1
        }
      else
        cargo build -p $WORKSPACE -r --offline --target $TARGET || {
          echo -e "\033[31mCargo build failed for package $WORKSPACE! ❌\033[0m"
          exit 1
        }
      fi
    else
      cargo build --workspace -r --offline --target $TARGET || {
  ...
  # fetch all .exe(s) inside target/release/* only, dont recursively
